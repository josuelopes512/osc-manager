...
export const upsertHandlers: Record<string, (item: any) => Promise<void>> = {
	students: async (student) => {
		try {
			...
			if (
				!student.course ||
				!student.course.name ||
				typeof student.course.name !== "string"
			) {
				throw new Error(
			...
			if (error instanceof Error) {
				console.error(`Erro no processamento do estudante: ${error.message}`);
				throw new Error(
			...
			throw new Error(
				`Erro desconhecido ao processar o estudante "${student.name}".`,
			);
		}
	},
	...
	projects: async (project) => {
		try {
			...
			if (error instanceof Error) {
				console.error(`Erro no processamento do projeto: ${error.message}`);
				throw new Error(
			...
			throw new Error(
				`Erro desconhecido ao processar o projeto "${project.name}".`,
			);
		}
	},
